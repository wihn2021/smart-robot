...
class Agent:
    ...
    def __init__(self, start_iter = 0) -> None:
        ...
        self.score = 0
        self.destinations = [(215,55),(130,55),(139,245),(220,240)]
        self.destination_iterator = start_iter
        self.observation_counter = 0
        self.fastforward_flag = 0
    ...
    def observe_single(self, bias) -> Tuple[int, Union[ndarray, None], Union[float, int, None]]:
        ...
        if tags == None:
            ...
            for tag in tags:
                ...
                if tag.tag_id < 37 and not (tag.corners[3,0] < 100 or tag.corners[3,1] > 400):
                    ...
                    if abs(average_angle - tag_ori[tag.tag_id]) % 180 < (90 + thres) and  abs(average_angle - tag_ori[tag.tag_id]) % 180 > (90 - thres) :
                        self.logger.warn("abs(average_angle - tag_ori[tag.tag_id]) % 180 < (90 + thres) and  abs(average_angle - tag_ori[tag.tag_id]) % 180 > (90 - thres)")
                        continue
#                     if linalg.norm(tag_poses[tag.tag_id][0][:2] - position[:2]) > 70:
#                         self.logger.warn("linalg.norm(tag_poses[tag.tag_id][0][:2] - position[:2])")
#                         continue
    ...
    def action_frame(self):
        ...
        try:
            if self.observation_counter % 4 == 0 or self.fastforward_flag == 1:
                self.fastforward_flag = 0
        ...
        try:
            ...
            if tag_num == 0:
                self.action.turn_head_right()
                tag_num, position, angle = self.observe_single(-75)
                self.action.turn_head_back()
                if tag_num == 0:
            ...
            if abs(should_rotate) > 15:
                ...
                d = linalg.norm([delta_x, delta_y])
                if d >= 3:
                    self.fastforward_flag = 1
                self.action.basic_go_forward(d)
